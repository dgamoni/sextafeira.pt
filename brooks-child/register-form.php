...
<div class="tml tml-register" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="registerform" id="registerform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'register', 'login_post' ); ?>" method="post">
		...
		<p class="tml-user-login-wrap">
			<label for="user_login<?php $template->the_instance(); ?>"><?php _e( 'Name', 'theme-my-login' ); ?></label>
			<input type="text" name="user_login" id="user_login<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'user_login' ); ?>" size="20" />
		</p>
		<?php endif; ?>

		<p class="tml-user-email-wrap">
			<label for="user_email<?php $template->the_instance(); ?>"><?php _e( 'E-MAIL ADRESSE', 'theme-my-login' ); ?></label>
			<input type="text" name="user_email" id="user_email<?php $template->the_instance(); ?>" class="input" value="<?php $template->the_posted_value( 'user_email' ); ?>" size="20" />
		</p>

		<?php do_action( 'register_form' ); ?>
        
        		<p class="tml-user-role-wrap">
        	<!--<label for="user_role<?php $template->the_instance(); ?>"><?php _e( 'User Role', 'theme-my-login' ); ?></label>-->
            <select name="role">
                <option value="">User Role</option>
             <?php /*?> <option value="artists">Artists</option><?php */?>
